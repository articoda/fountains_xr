...
# Import gaussian_filter for smoothing presentation maps.
from scipy.ndimage import gaussian_filter


def smooth_accessibility_share_for_plot(
    ...
    """
    Smooth an accessibility-share map for visualization.

    This should be used only for plotting, not for numerical analysis.
    The smoothing is population-weighted because we smooth numerator
    and denominator separately.
    """

    # Smooth the accessible population.
    accessible_smooth = gaussian_filter(
        accessible_population_agg.astype("float32"),
        sigma=sigma
    )

    # Smooth the total population.
    total_smooth = gaussian_filter(
        total_population_agg.astype("float32"),
        sigma=sigma
    )

    # Prepare empty output.
    share_smooth = np.full(
        total_smooth.shape,
        np.nan,
        dtype="float32"
    )

    # Keep only cells with enough smoothed population.
    valid = total_smooth >= min_total_population

    # Compute percentage.
    share_smooth[valid] = (
        accessible_smooth[valid]
        / total_smooth[valid]
        * 100
    )
...
def plot_accessibility_share_raster(
    ...
    # Plot the selected area as a light grey background.
    boundary_for_plot.plot(
        ax=ax,
        color="lightgrey",
        edgecolor="none",
        zorder=0
    ...
    # Plot the accessibility-share raster.
    img = ax.imshow(
        np.ma.masked_invalid(accessibility_share),
        extent=extent,
        origin="upper",
        cmap=cmap,
        vmin=0,
        vmax=100,
        interpolation="bilinear",   # presentation only, for the “strict/data” plot, use nearest
        zorder=1
    )

    # Draw the boundary on top.
    boundary_for_plot.boundary.plot(
        ax=ax,
        linewidth=0.8,
        color="black",
        zorder=2
...
def choose_plot_settings(analysis_level):
    ...
    if analysis_level == "italy":
        return {
            "aggregation_factor": 50,      # 5 km
            "min_total_population": 500,
            "hexbin_gridsize": 150,
            "cell_label": "5km",
            "plot_population_map": False
        }

    if analysis_level == "region":
        return {
            "aggregation_factor": 10,      # 2 km
            "min_total_population": 100,
            "hexbin_gridsize": 80,
            "cell_label": "1km",
            "plot_population_map": False
